...
class Pacjent:
    def __init__(self, id, canvas):
        ...
        self.x, self.y = 0, 0
        self.czas_na_odziale = random.randint(300, 600)
        ...
        for idx, oddzial in enumerate(self.oddzialy.values()):
            ...
            for i, pacjent in enumerate(oddzial.lozka):
                ...
                if isinstance(pacjent, PacjentNaBadaniu):
                    ...
                    self.canvas.itemconfig(rect_id, fill=pacjent_actual.color)
                    # Dodaj "X" na środku łóżka
                    lx, ly = self.lozka_graficzne[(nazwa, i)]
                    self.canvas.create_text(lx + 8, ly + 10, text="X", font=("Arial", 10, "bold"))
                elif isinstance(pacjent, Pacjent):
                    pacjent_actual = pacjent
                    self.canvas.itemconfig(rect_id, fill=pacjent.color)
                else:
                ...
                if pacjent_actual:
                    ...
                    if isinstance(oddzial.lozka[i], Pacjent):
...
if __name__ == "__main__":
    try:
        ...
        print("\n⛔ Przerwano przez użytkownika (Ctrl+C). Zamykanie programu...")
        app.zakoncz_program()
